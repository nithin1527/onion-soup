-- Define the Ipelet
label = "Random Trapezoidal Map"
about = "Returns a Randomly Generated Trapezoidal Map"
...
function run(model)
    ...
    math.randomseed(math.random(1, 1000000)) -- Seed the random generator
    ...
    for i = 1, 5 do
        local x1 = random_range(100, 500)
        local x2 = random_range(100, 500)
        local y1 = random_range(100, 500)
        local y2 = random_range(100, 500)

        -- Ensure x1 <= x2 and y1 <= y2 for consistency
        if x1 > x2 then x1, x2 = x2, x1 end
        if y1 > y2 then y1, y2 = y2, y1 end

        table.insert(inpt, {{x1, x2}, {y1, y2}})
    end

    -- Find min and max values for the last entry
    local x_min, x_max = math.huge, -math.huge
    local y_min, y_max = math.huge, -math.huge

    for _, rect in ipairs(inpt) do
        x_min = math.min(x_min, rect[1][1], rect[1][2])
        x_max = math.max(x_max, rect[1][1], rect[1][2])
        y_min = math.min(y_min, rect[2][1], rect[2][2])
        y_max = math.max(y_max, rect[2][1], rect[2][2])
    end

    -- Append the bounding box to inpt
    table.insert(inpt, {{x_min, x_max}, {y_min, y_max}})

    for i = 1, #inpt-1 do
        local start = ipe.Vector(inpt[i][1][1],inpt[i][2][1])
        local finish = ipe.Vector(inpt[i][1][2],inpt[i][2][2])
